...
## Overview
`BigFloat` is the numeric core of **Expidle**, a test‑bed for building an F#‑first game engine/framework tailored to 
idle and incremental games. It is designed to feel natural in **F#**, predictable in **C#**, and as close as practical
to **IEEE‑754 floating‑point semantics**, while supporting numbers far beyond the range of `double`.
...
It is not a drop‑in replacement for `double`, but a practical numeric type for 
game logic.
...
This “semantic tri‑bridge” allows `BigFloat` to behave naturally in both 
languages while remaining IEEE‑inspired.

### `.Equals` and `GetHashCode` (the .NET world)
